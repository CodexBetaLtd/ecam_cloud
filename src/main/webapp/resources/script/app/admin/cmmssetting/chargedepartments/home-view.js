var ChargeDepartmentHome = function() {
	...
	var runDataTable = function() {
		...
		var oTable = $('#chargeDepartmentTbl').DataTable( {
			...
			"ajax" : $.fn.dataTable
					.pipeline({
						url : "../restapi/lookuptable/tabledataChargeDepartment",
						pages : 5
					}),
			columns : [
					{ 
			...
			dom : "<'row'<'col-sm-4 dtblchargeDepartment'><'col-sm-8'f>>"
					+ "<'row'<'col-sm-12'tr>>"
		...
		$('#chargeDepartmentTbl_wrapper .dataTables_length select').addClass("m-wrap small");
	...
	var addModal = function() {
		...
		setTimeout(function() {
			var url = '../chargeDepartment/add';
			$modal.load(url, '', function() {
				$modal.modal();
				ChargeDepartmentAdd.init();
			});
		}, 1000);
	};
	...
	var editModal = function(id) {
		...
		$.ajax({
			url : '../chargeDepartment/edit?id=' + id,
			type : 'GET',
	...
	var saveModal = function() {
		var $modal = $('#cmms-setting-add-modal');
		var form = $('#chargeDepartmentAddForm');
	...
	var deleteInsideModal = function(id) {
		var $modal = $('#cmms-setting-add-modal');
		$.ajax({
			url : '../chargeDepartment/delete?id=' + id,
			type : "GET",
			success : function(response) {
				closeModal();
				runDataTable();
