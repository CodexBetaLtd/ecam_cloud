var MaintenanceTypeHome = function() {
	...
	var runDataTable = function() {
		...
		var oTable = $('#maintenanceTypeTbl').DataTable( {
			...
			dom : "<'row'<'col-sm-4 dtblmaintenanceType'><'col-sm-8'f>>"
					+ "<'row'<'col-sm-12'tr>>"
		...
		$('#maintenanceTypeTbl_wrapper .dataTables_length select').addClass(
				"m-wrap small");
	...
	var addModal = function() {
		...
		setTimeout(function() {
			var url = '../maintenanceType/add';
			$modal.load(url, '', function() {
				$modal.modal();
				MaintenanceTypeAdd.init();
			});
		}, 1000);
	};
	...
	var editModal = function(id) {
		...
		$.ajax({
			url : '../maintenanceType/edit?id=' + id,
			type : 'GET',
	...
	var saveModal = function() {
		var $modal = $('#cmms-setting-add-modal');
		var form = $('#maintenanceTypeAddForm');
	...
	var deleteModal = function(id) {
		...
		$.ajax({
			url : '../maintenanceType/delete?id=' + id,
			type : 'GET',
			success : function(response) {
				runDataTable();
			},
