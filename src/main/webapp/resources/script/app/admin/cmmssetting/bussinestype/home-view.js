var BussinessTypeHome = function() {
	...
	var runDataTable = function() {
		...
		var oTable = $('#bussiness_type_tbl').DataTable( {
			...
			initComplete : function() {
				$("div.dtblbusinessType")
                        ...
                        "<button class='btn btn-default btn-sm active tooltips' data-toggle='modal' type='button' id='businessTypeDelete'><i class='clip-cancel-circle-2 btn-delete'></i> Delete </button>"
                        );
			},
           ...
           rowClick : {
                sFunc: "BussinessTypeHome.editModal",
                aoData:[  
                    {
                        sName : "id",
                    },
        ...
        DataTableUtil.deleteRowsFunc(oTable, "businessTypeDelete", "BussinessTypeHome.deleteMutiple", "id");
	};
    ...
    var deleteMutiple = function(ids) {
        $.ajax({
            url: "../businessType/delete-multiple?ids="+ ids,
            type: 'GET',
            success: function(response) {
                $("#collapseThree").find('.panel-body').empty().append(response);
