var BusinessClassificationHome = function() {
	...
	var runDataTable = function() {
		...
		var oTable = $('#businessClassificationTbl').dataTable( {
			...
			"ajax" : $.fn.dataTable.pipeline({
						url : "../restapi/lookuptable/tabledataBusinessClassification",
						pages : 5
					}),
			columns : [
					{
			...
			dom : "<'row'<'col-sm-4 dtblbusinessClassification'><'col-sm-8'f>>"
					+ "<'row'<'col-sm-12'tr>>"
		...
		$('#businessClassificationTbl_wrapper .dataTables_filter input').addClass("form-control input-sm").attr("placeholder", "Search");
		// modify table search input
		$('#businessClassificationTbl_wrapper .dataTables_length select').addClass("m-wrap small");
		// modify table per page dropdown
		$('#businessClassificationTbl_wrapper .dataTables_length select').select2();
		// initialzie select2 dropdown
		$('#businessClassificationTbl_columnToggler input[type="checkbox"]').change(function() {
	...
	var addModal = function() {
		...
		setTimeout(function() {
			var url = '../businessClassification/add';
			$modal.load(url, '', function() {
				$modal.modal();
				BusinessClassificationAdd.init();
			});
		}, 1000);
	};
	...
	var editModal = function(id) {
		...
		$.ajax({
			url : '../businessClassification/edit?id=' + id,
			type : 'GET',
	...
	var saveModal = function() {
		var $modal = $('#cmms-setting-add-modal');
		var form = $('#businessClassificationForm');
	...
	var deleteModal = function(id) {
		console.log(id);
		$.ajax({
			url : '../businessClassification/delete?id=' + id,
			type : 'GET',
			success : function(response) {
				runDataTable();
			},
