TabWorkOrder = function () {
	...
	var runWorkOrderDataTable = function (projectId) {
        var oTable = $('#work_order_tbl').dataTable({
            processing: true,
            serverSide: true,
            ajax: $.fn.dataTable.pipeline({
                url: "../restapi/workorder/tableDataByProject",
                pages: 5,
                data: {
                    "id": projectId
                },
        ...
        $('#work_order_tbl_wrapper .dataTables_length select').select2();

        $('#work_order_tbl_column_toggler input[type="checkbox"]').change(function () {
	...
	return {
		runWorkOrderDataTable : function (projectId) {
			runWorkOrderDataTable(projectId);
		}
    };
}();