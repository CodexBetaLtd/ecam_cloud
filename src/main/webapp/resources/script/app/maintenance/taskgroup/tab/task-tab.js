var TaskTab = function () {  
    ...
    var taskAddModal = function () {
        ...
        setTimeout(function () {
            var url = '../taskGroup/task-add-modal-view';
    ...
    var initTaskTable = function () {
        ...
        if (tasks.length > 0) {
            $.each(tasks, function (index, task) {
                ...
                $('#task-tbl > tbody:last-child').append(row);
            });
        } else {
            CustomComponents.emptyTableRow("task-tbl", 6, "Please Add Tasks for the task Group.");
        }
    };
