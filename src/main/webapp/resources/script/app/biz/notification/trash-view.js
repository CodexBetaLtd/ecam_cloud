var TrashNotification = function () {
    ...
    var dtTrash = function () {
        var tableId = "tbl_notification_trash_list";
        $('#' + tableId).dataTable().fnDestroy();
        var oTable = $('#' + tableId).dataTable({
        	"processing": true,
            "serverSide": true,
            "ajax": $.fn.dataTable.pipeline( {
            	url : "../restapi/notification/trash",
            	pages: 10
            } ),            
            columns : [
                {
    ...
    var getNotificationButtonSet = function (id) {
        ...
        var modalIdUndo = "not_view-modal_" + id;
        return "<div class='message-time' style='position: relative'>  " +
            ...
            "<span><a class='btn btn-xs btn-warning' data-placement='top' onclick='TrashNotification.previewNotification(" + id + ")' type='button' data-toggle='modal' title='Preview'> <i class='fa fa-laptop'></i> </a> </span>" +
            // "<a class='btn btn-xs btn-danger' type='button' data-toggle='modal' title='Preview' tabindex='-1' href='#" + modalIdUndo + "'> <i class='fa fa-laptop'></i> </a> " +
            "<span><a class='btn btn-xs btn-light-grey ' type='button' data-toggle='modal' title='Undo From Trash' tabindex='-1' href='#" + modalIdUndo + "'> <i class='fa fa-undo'></i> </a> </span> " +
            "<span><a class='btn btn-xs btn-danger' type='button' data-toggle='modal' title='Delete ' tabindex='-1' href='#" + modalId + "'> <i class='fa fa-close'></i> </a>  </span>" +
            "</div> " +
            ...
            "<a href='../notification/undoTrashItem?id=" + id + "' class='btn btn-sm btn-red'> Trash </a>" +
            "</div>" +
            "</div>" +

            "<div id='" + modalIdUndo + "' class='modal fade delete-center-modal' tabindex='-1' data-backdrop='static' data-keyboard='false' style='display: none; '>" +
            "<div class='modal-body'>" +
            "<p>Are you sure, You are going to permanent delete. Do you want to proceed this ?</p>" +
            "</div>" +
            "<div class='modal-footer'>" +
            "<a data-dismiss='modal' class='btn btn-blue btn-sm' > Cancel</a> " +
            "<a href='../notification/delete?id=" + id + "' class='btn btn-sm btn-red'> Trash </a>" +
            "</div>" +
            "</div>" +
    ...
    var getReceiver = function (dataSet) {
        if (dataSet.systemMessage == true || dataSet.systemMessage == "true" || dataSet.systemMessage === 'true') {
            return "<span class='messages-item-to'> <strong> To :  [" + dataSet.receiverName + "] </strong></span>";
        } else {
            return "<span class='messages-item-to'> <strong> To : </strong> " + dataSet.receiverName + "</span>";
        }
    };
    ...
    var initHideButton = function () {
        $("#btn-notification-reply").hide();
        $("#btn-notification-forward").hide();
        $("#btn-notification-trash").hide();
    };
