var IssueNoteItemTab = function () {
    
    /**********************************************************
     * Init Buttons
     *********************************************************/
    var initButtons = function () {
        $('#btn-issue-note-item-modal-view').on('click', function () {
        	IssueNoteItemTab.issueNoteItemView();
        });
    };
    ...
    var issueNoteItemView = function () {
        var $modal = $('#master-modal-datatable');
        CustomComponents.ajaxModalLoadingProgressBar();
        setTimeout(function () {
            var url = '../issuenote/issenoteitemview';
            $modal.load(url, '', function () {
            	IssueNoteItemAddModal.init('master-modal-datatable');
                $modal.modal();
            });
        }, 1000);
    };  
                ...
                var html = "<tr id='row_" + row + "' >" +
                    ...
                    "<td class='center'>" +
		                "<div class='visible-md visible-lg hidden-sm hidden-xs'>" +
			                ButtonUtil.getCommonBtnEdit("IssueNoteItemTab.editListItem",  row) +
			                ButtonUtil.getCommonBtnDelete("IssueNoteItemTab.deleteListItem", row) +
		                "</div>" +
	                "</td>"+
                    "</tr>";
                $('#tbl_issuenote_item > tbody:last-child').append(html);
            }
        } else {
            CustomComponents.emptyTableRow("tbl_issuenote_item", 8);
        }
    };
