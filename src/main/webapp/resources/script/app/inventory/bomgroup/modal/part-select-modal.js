var PartSelectModal = function () {
    ...
    var addPart = function (id, name) {
        if (!isPartAlreadyAdd(id)) {
            addPartToList(id, name);
            PartTab.resetPartHtmlTable();
        }
        $('#'+ this.modalName).modal('toggle');
    };

    var isPartAlreadyAdd = function (id) {
    ...
    var addPartToList = function (id, name) {
        var partObj = {}
        partObj['id'] = id;
        partObj['name'] = name;
        parts.push(partObj);
