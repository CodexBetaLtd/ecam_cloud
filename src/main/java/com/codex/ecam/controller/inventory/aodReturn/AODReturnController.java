...
public class AODReturnController {
	...
	public String delete(Integer id, Model model, RedirectAttributes ra) {
		try {
			final AODReturnResult result = aodReturnService.delete(id);
			if (result.getStatus().equals(ResultStatus.ERROR)) {
	...
	public String getUnFinalizedAODReturns(Model model, RedirectAttributes ra) {
		try {
			final List<AODReturnDTO> dtos = aodReturnService.getUnFinalizedAODReturns();
	...
	public String deleteMultiple(Integer ids[], Model model) {
		...
		try {
			final AODReturnResult result = aodReturnService.deleteMultiple(ids);
			if (result.getStatus().equals(ResultStatus.ERROR)) {
			...
			model.addAttribute("error", "AOD Return already assigned. Please remove from where assigned and try again.");
		}  catch (final Exception e) {
