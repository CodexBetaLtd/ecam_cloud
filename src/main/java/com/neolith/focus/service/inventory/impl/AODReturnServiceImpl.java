...
import com.neolith.focus.constants.util.AffixList;
import com.neolith.focus.dao.asset.AssetDao;
import com.neolith.focus.dao.biz.BusinessDao;
import com.neolith.focus.dao.inventory.AODDao;
import com.neolith.focus.dao.inventory.AODItemDao;
import com.neolith.focus.dao.inventory.AODReturnDao;
...
import com.neolith.focus.dto.inventory.aodReturn.AODReturnItemDTO;
import com.neolith.focus.dto.inventory.aodReturn.AODReturnRepDTO;
import com.neolith.focus.mappers.inventory.aodReturn.AODReturnItemMapper;
import com.neolith.focus.mappers.inventory.aodReturn.AODReturnMapper;
import com.neolith.focus.mappers.inventory.aodReturn.AODReturnReportMapper;
...
import com.neolith.focus.util.search.inventory.aodReturn.AODReturnPropertyMapper;
...
public class AODReturnServiceImpl implements AODReturnService {
	...
	private String getNextCode() {
		...
		AODReturn lastDomain = aodReturnDao.findLastDomain();
		if ((lastDomain == null) || (lastDomain.getId() == null)) {
			nextNo = 1;
		} else {
			List<String> codeList = Arrays.asList(lastDomain.getReturnNo().split("/"));
	...
	public AODReturnResult save(AODReturnDTO dto) throws Exception {
		AODReturnResult result = new AODReturnResult(new AODReturn(), dto);
		try {
			saveOrUpdate(result);
			result.setResultStatusSuccess();
	...
	public AODReturnResult update(AODReturnDTO dto) throws Exception {
		AODReturnResult result = new AODReturnResult(null, dto);
		try {
	...
	private void setAODReturnItem(AODReturnResult result) throws Exception {
		Set<AODReturnItem> returnItems = new HashSet<>();
		if ((result.getDtoEntity().getAodReturnItemList() != null) && (result.getDtoEntity().getAodReturnItemList().size() > 0)) {
			Set<AODReturnItem> currentItems = result.getDomainEntity().getAodReturnItems();
			for (AODReturnItemDTO aodItemDTO : result.getDtoEntity().getAodReturnItemList()) {
				AODReturnItem returnItem;
				if ((currentItems != null) && (currentItems.size() > 0)) {
					Optional<AODReturnItem> optional = currentItems.stream().filter((x) -> x.getId().equals(aodItemDTO.getId())).findAny();
	...
	private Set<AODReturnItemDTO> aodReturnItemByAODItem() throws Exception {
		Set<AODReturnItemDTO> aodReturnItemDTOList = new HashSet<>();
		for (AODReturnItemDTO returnDTO : aodReturnItemDTOList) {
			AODReturnItemDTO itemDTO = new AODReturnItemDTO();
	...
	public List<AODReturnDTO> getUnFinalizedAODReturns() {
		List<AODReturnDTO> dtos = new ArrayList<>();
		List<AODReturn> aodReturns = aodReturnDao.findAODReturnByStatus(AODReturnStatus.DRAFT.getId());
		try {
			for (AODReturn aodReturn : aodReturns) {
	...
	public List<AODReturnRepDTO> findAll(AODReturnFilterDTO aodReturnFilterDTO) {
		...
		try {
			Specification<AODReturn> specification = (root, query, cb) -> {
	...
	public DataTablesOutput<AODReturnDTO> findAll(FocusDataTablesInput input) throws Exception {
		...
		if (AuthenticationUtil.isAuthUserAdminLevel()) {
			...
			Specification<AODReturn> specification = (root, query, cb) -> cb.equal(root.get("business"), AuthenticationUtil.getLoginUserBusiness());
			domainOut = aodReturnDao.findAll(input, specification);
		} else {
			Specification<AODReturn> specification = (root, query, cb) -> cb.and(
		...
		DataTablesOutput<AODReturnDTO> out = AODReturnMapper.getInstance().domainToDTODataTablesOutput(domainOut);
		return out;
	}
