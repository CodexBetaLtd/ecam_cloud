...
import com.neolith.focus.service.notification.api.NotificationRecipientBuilderService;
import com.neolith.focus.support.notification.NotificationInternalSender;
...
public class StockNotificationServiceImpl implements StockNotificationService {
	...
	@Override
	public void publishMinQtyNotifications(Stock domain) throws Exception {
		List<Integer> recipients = notificationRecipientBuilderService.getStockMinQtyNotifyRecipients(domain);
		String subject="Stock Min Quatity Reached";
		sendNotifications(subject,domain,recipients);
	}
	...
	private void setRecipientData(Integer recipientId, NotificationDTO dto) { 
        ...
        if (recipient.getEmailNotification() == true) {
            if (recipient.getEmailAddress() != null) {
				...
				dto.setReceiverName(recipient.getFullName());
			}
		}  
